...
library_path = "/usr/local/lib/libhidapi.dylib"
try:
    hidapi = ctypes.cdll.LoadLibrary(library_path)
except OSError as error:
    raise ImportError(f"Couldn't load {library_path}\n Error: {error}")

hidapi.hid_init()
register(hidapi.hid_exit)
...
class Device(object):
    ...
    def write(self, data):
        ...
        binairy_data = ctypes.c_char_p(bytes_data)
        return self.__hidcall(hidapi.hid_write, self.__dev, binairy_data, len(data))
