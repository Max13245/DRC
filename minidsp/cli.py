...
def main():
    ...
    parser.add_argument(
        "control", type=str, help="Control (" + ", ".join(controls) + ")"
    )
    parser.add_argument(
        "value", type=str, nargs="?", help="Value to set, if action is 'set'"
    )
    parser.add_argument(
        "-t",
        "--transport",
    ...
    if args.action == "get":
        ...
        if args.control == "gain":
            try:
                ...
                parser.error(
                    "Gain must be provided in db and without units ('-127.5' to '12')"
                )
